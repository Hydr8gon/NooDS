...
Core::Core(): cart9(&dma9, &arm9, &memory), cart7(&dma7, &arm7, &memory), cp15(&arm9), dma9(true, &arm9, &memory),
              ...
              arm9(&cp15, &memory), arm7(&memory), ipc(&arm9, &arm7), memory(&cart9, &cart7, &cp15, &dma9, &dma7,
              &gpu, &engineA, &engineB, &gpu3D, &gpu3DRenderer, &input, &arm9, &arm7, &ipc, &math, &rtc, &spi,
              &spu, &timers9, &timers7, &wifi), spi(&arm7, firmware), spu(&memory), timers9(&arm9), timers7(&arm7)
{
