...
void HleArm7::runFrame() {
    ...
    if (!inited) return;
    core->memory.write<uint16_t>(1, 0x2FFFFA8, (core->input.readExtKeyIn() & 0xB) << 10);
...
void HleArm7::pollTouch(uint32_t value) {
    ...
    if (core->input.readExtKeyIn() & BIT(6)) { // Released
        ...
        core->memory.write<uint16_t>(1, 0x2FFFFAA, (core->spi.touchX >> 0));
        core->memory.write<uint16_t>(1, 0x2FFFFAC, (core->spi.touchY >> 4) | 0x100);
    }
